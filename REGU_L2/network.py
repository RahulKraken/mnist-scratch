...
class Network():
  ...
  def SGD(self, training_data, batch_size, epochs, eta, lamda, test_data=None):
  ...
  def update_mini_batch(self, mini_batch, eta, lamda, n):
    ...
    self.weights = [(1 - eta * (lamda / n)) * w - (eta / len(mini_batch)) * dnw for w, dnw in zip(self.weights, total_dw)]
